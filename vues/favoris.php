...
if(!empty($_SESSION['idUtilisateur'])){
    if(!isset($_GET["all"])){
        $Favoris = new Favoris();
        $limitFavoris = $Favoris->getFiveFavorisForUser($_SESSION['idUtilisateur']);
        ?>
        <div id="main-favoris-container">
            <div id="favoris-container" class="my-3 pb-3">

        <?php
        if(count($limitFavoris) > 0){

            foreach($limitFavoris as $favoris){
            ...
            <div class="d-flex justify-content-center">
                <a href="?all"><button class="btn btn-primary">Tout afficher</button></a>
            </div>
            </div>
